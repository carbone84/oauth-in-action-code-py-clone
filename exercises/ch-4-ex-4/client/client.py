...
favorites_api = 'http://localhost:5002/favorites'
...
def favorites():
  ...
  resource = requests.get(favorites_api, headers=headers)

  if resource.status_code >= 200 and resource.status_code < 300:
    body = resource.json()
    print(f"body {body}")
    return render_template('favorites.html', scope=session['scope'], data=body)
  else:
    return render_template('favorites.html', scope=session['scope'], data={'user': '', 'favorites': {'movies': [], 'foods': [], 'music': []}})

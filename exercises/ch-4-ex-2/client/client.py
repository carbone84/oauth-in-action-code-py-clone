...
word_api = 'http://localhost:5002/words'
...
@app.route('/words')
def words():
  return render_template('words.html', words="", timestamp=0, result="noget")
...
def get_words():
  ...
  resource = requests.get(word_api, headers=headers)

  if resource.status_code >= 200 and resource.status_code < 300:
    body = resource.json()
    return render_template('words.html', words=body['words'], timestamp=body['timestamp'], result="get")
  else:
    return render_template('words.html', words="", timestamp=0, result="noget")
...
def add_word():
  ...
  resource = requests.post(word_api, headers=headers, data=form_body)
  
  if resource.status_code >= 200 and resource.status_code < 300:
    return render_template('words.html', words="", timestamp=0, result="add")
  else:
    return render_template('words.html', words="", timestamp=0, result="noadd")
...
def delete_word():
  ...
  resource = requests.delete(word_api, headers=headers)
  
  if resource.status_code >= 200 and resource.status_code < 300:
    return render_template('words.html', words="", timestamp=0, result="rm")
  else:
    return render_template('words.html', words="", timestamp=0, result="norm")
